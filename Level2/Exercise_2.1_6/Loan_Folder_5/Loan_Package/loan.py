'''
This class improves upon the base class in Loan Folder 3 by adding static methods (unchanged for Loan_Folder_5, last change is in Loan_Folder_4). Version 2.
No retroactive edits from getRate. getRate problems can be handled via exception handler.
'''
...
class Loan(object):
    ...
    def interestDue(self, t):
        ...
        if self._rate <= 0 or self._term <= 0 or self._notional <= 0:
            ...
            mRate = Loan.monthlyRate(self._rate)  # (Calc to Static 8.) Originally a calculation, Now a Static Method ... Be careful! If you set rate above balance, you get the wrong values.
    ...
    def interestDueRecursive(self, t):
        ...
        if self._rate <= 0 or self._term <= 0 or self._notional <= 0:
            ...
            if not t:
                mRate = Loan.monthlyRate(self._rate) # (Calc to Static 6.) Originally a calculation, Now a Static Method ...
    ...
    def principalDueRecursive(self, t):
        ...
        if self._rate <= 0 or self._term <= 0 or self._notional <= 0:
            ...
            monthlyPay = self.monthlyPayment()
            if not t:
                return monthlyPay - Loan.monthlyRate(self._rate) * self._notional # (Calc to Static 4.) Originally a calculation, Now a Static Method ...
            else:
                interestDue = self.balanceRecursive(t) * Loan.monthlyRate(self._rate) # (Calc to Static 5.) Originally a calculation, Now a Static Method ...
    ...
    def balanceRecursive(self, t):
        ...
        if self._rate <= 0 or self._term <= 0 or self._notional <= 0:
            ...
            if not t:
                ...
                return self._notional - (payment - Loan.monthlyRate(self._rate) * self._notional) # (Calc to Static 3.) Originally a calculation, Now a Static Method
            else:
