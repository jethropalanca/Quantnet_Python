'''
This program tests the new loan type functionalities, and tests if the old functions work after modifying them.
Version 2: VariableRateLoan won't work as some monthlypay functions were left blank.
Version 3: GOOD CALL TO TEST VARIABLE RATE LOAN AS WELL!. Added t for each, Fixed Line 181 and changed it to getRate as it cannot take a self._rate
Version 4: Found a work-around for the recursion problem my declaring the things I don't want to recurse outside the an outer function, before running the recursive functions.
