...
class Timer(object):
    ...
    def __enter__(self):
        logging.info('The timer is starting...')
        self.start()
        return self
        ...
        if not type: # Deal with other errors:
            ...
            if config == 'seconds' or config == 'minutes' or config == 'hours':
                ...
                if timeResult > warnThreshold: # 4_2_2: If within threshold, INFO
                    ...
                    logging.info('This process is within the warning threshold...')
                    logging.info(f'{name}: {adjtimeResult:.8f} vs. {warnThreshold * cnfgN:.8f} {config} (actual vs. threshold)') # Initialized at the beginning, manualy re-initialized at exit for default-setting.
            else:
