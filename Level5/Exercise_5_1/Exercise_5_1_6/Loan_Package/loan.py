...
from Exercise_5_1.Exercise_5_1_6.Loan_Package.asset import Asset
import logging
import datetime
...
class Loan(object):
    ...
    def __init__(self, startmaturity, endmaturity, rate, notional, asset):
        ...
        self.getTerm = self.getTerm() # 5_1_6 to get new term to work
    ...
    def calcMonthlyPmt(cls, term, rate, notional):
        ...
        :param term: Derived from self.term() and is calculated outside the function. ALREADY MONTHLY in 5_1_6 so no need to get monthly term.
        :param rate: Annualized rate
        :param notional: Loan face value
        '''
        ...
        if rate <= 0 or term <= 0 or notional <= 0:
            ...
            # No Logging statements here else will be double-logged in the functions... Commented in 4_2_3
            # No need for monthly for 5_1_6, already in months
            # mTerm = term * 12
            ...
            return (mRate * notional) / (1 - (1 + mRate) ** (-term))
    ...
    def getTerm(self):
        ...
        if isinstance(self._startmaturity, datetime.datetime) and isinstance(self._endmaturity, datetime.datetime):
            logging.debug('Subtracting the two dates...')
            dateDifference = self._endmaturity - self._startmaturity
    ...
    def totalPayments(self):
        ...
        if not payment:
            ...
            return sum([self.monthlyPayment(t) for t in range(self._term)])
