'''
FINAL VERSION for final project: from datetime version to PERIOD.

Adjusted for APalley's Comments - getTerm is now a STATIC Method

Version 5_1_6. Applied StartMaturity and End maturity. Fixed Total Payments per APalley Comments in Level 3.
...
class Loan(object):
    ...
    def __init__(self, term, rate, notional, asset):
        ...
        # For Final Project, new class variables:
        self._defProb = 0 # default probability
        self._hasDefaulted = False
    '''
    def __init__(self, startmaturity, endmaturity, rate, notional, asset):
        
        This is the initialization function for the loan class:
        Latest Version: Fixed the Zero Division Error!
        :param startmaturity: Takes a loan start date.
        :param endmaturity: Takes a loan end date (Maturity).
        :param rate: Annualized rate
        :param notional: Loan face value
        :param asset: Asset Value
        

        self._startmaturity = startmaturity
        self._endmaturity = endmaturity
        self._rate = rate
        self._notional = notional
        self._term = Loan.getTerm(startmaturity, endmaturity)
        ...
        if isinstance(asset, Asset):
            ...
            # 'Ensure that the asset parameter indeed contains an Asset object / any of its derived classes'
            logging.error('\nPlease enter an appropriate Asset class in the fifth parameter. Exiting...') # Changed in 4_2_3
            raise TypeError('Please enter an appropriate Asset class in the fifth parameter. Exiting...') # Changed in 4_2_3
    '''
    '''
    @property
    def startmaturity(self):
        return self._startmaturity

    @startmaturity.setter
    def startmaturity(self, istartmaturity):
        self._startmaturity = istartmaturity

    @property
    def endmaturity(self):
        return self._endmaturity

    @endmaturity.setter
    def endmaturity(self, iendmaturity):
        self._endmaturity = iendmaturity
    '''
    ...
    # For Final Project: default Probability
    @property
    def defProb(self):
        return self._defProb

    @defProb.setter
    def defProb(self, idefProb):
        self._defProb = idefProb
    ...
    ##############################
    '''
    # (5.1_6) New method for term calculation based on inputted start_date and end_date:
    
    @staticmethod
    def getTerm(startmaturity, endmaturity):
        logging.debug('\nProcessing Date Differential...')
        if isinstance(startmaturity, datetime.datetime) and isinstance(endmaturity, datetime.datetime):
            logging.debug('Subtracting the two dates...')
            dateDifference = endmaturity - startmaturity

            logging.debug('Converting total seconds to months and then rounding off...')
            monthDifference = dateDifference.total_seconds() / 2592000 # 2592000 seconds in a month
            monthDifference = round(monthDifference)

            return monthDifference

        else:
            raise ValueError('Wrong input for BASE DATE or COMPARISON DATE. please try again...')
    '''
    ...
    def totalPayments(self):
        ...
        if not payment:
            ...
            # No need to multiply by 12 anymore... (already in months)
            # return sum([self.monthlyPayment(t) for t in range(self._term)]) # multiply by 12 to get monthly
            return sum(self.monthlyPayment(t) for t in range(self._term))  # GENERATOR: multiply by 12 to get monthly
    ...
    def interestDue(self, t):
        ...
        # if t > (self._term * 12):
        #     logging.debug(f'NOTICE: period is greater than term for {self}...')
    ...
    def interestDueRecursive(self, t):
        ...
        # Inform user that period should be within term
        if t > (self._term): # NEED AGAIN... (x) no need to multiply by twelve anymore
            logging.debug(f'NOTICE: period is greater than term for {self}...')
    ...
    def principalDueRecursive(self, t):
        ...
        # Inform user that period should be within term
        if t > (self._term): # no need to multiply by twelve anymore
            logging.debug(f'NOTICE: period is greater than term for {self}...')
    ...
    def balance(self, t):
        ...
        gRate = self.getRate(t)  # Modification for Variable rate loans
        balanceT = 0 if self._hasDefaulted == True else self.calcBalance(self._term, gRate, self._notional, t)

        # return self.calcBalance(self._term, gRate, self._notional, t) # calcBalance already has monthly rate function.
        # return 0 if self._hasDefaulted == True else self.calcBalance(self._term, gRate, self._notional, t) # Modification to include effect f has default flags
        return balanceT
    ...
    def recoveryValue(self, t):
        ...
        logging.debug(f'\nCalculating recovery value at t as asset value at t * {recoveryPCT}...')  # Changed in 4_2_3
        return self._asset.value(t) * recoveryPCT
    ...
    # ABS Project Formulas
    # convt Prob
    def convtProb(self, t):
        '''
        Returns the corresponding default probability of a passed-in time period.
        '''

        # Create Probability List
        probabilityDict = {10: 0.0005, 59: 0.001, 120: 0.002, 180: 0.004, 210: 0.002, 360: 0.001}
        probabilityList = list(probabilityDict.keys()) # use keys as we compare t with the key list, and then get the corresponding probability

        if t <= 0 or t > 360:
            self._defProb = 'N/A'

        else:
            # ratingsAnswer = [1 if t <= item else 0 for item in probabilityList]
            ratingsAnswer = [1 if t <= item else 0 for item in probabilityList]
            index = ratingsAnswer.index(1)  # 1
            self._defProb = 'N/A' if 1 not in ratingsAnswer else list(probabilityDict.values())[index]

        '''
        Too Messy... Removed.
        # Show Legend (PUT THIS AT THE END ELSE t = 360 for the lookup functions):
        logging.info(f'\nGetting the corresponding default probability for t = {t}.')
        logging.info('(*) Default Probability Table (For Reference):')
        for t, prob in probabilityDict.items():
            logging.info(f'Upto Time Period {t}: Default Probability equals {prob}')  # print ratings
        '''
        return self._defProb

    # checkDefault
    def checkDefault(self, numPut, t):
        '''
        Checks whether the loan defaults or not based on a passed-in random number from the Loan Pool.
        Returns RECOVERY VALUE of a loan at time t.
        :param numPut: Passed-in random number.
        '''

        if not numPut:
            self._hasDefaulted = True # Flag
            # Balance Set to 0

        # Recovery value is either 0.35 ir 0.4 in the spreadsheet
        # return (self._asset.value(t) * 0.4) if self._hasDefaulted is True else self._asset.value(t)

        # Actually, I already have a recovery value function from 4_2_3...
        # return (self._asset.value(t) * 0.4) if self._hasDefaulted is True else 0  # Return RECOVERY VALUE, NOT asset value
        # return self.recoveryValue(t) if self._hasDefaulted is True else 0  # Return RECOVERY VALUE, NOT asset value

        checkDefaultValue = self.recoveryValue(t) if self._hasDefaulted is True else 0  # Return RECOVERY VALUE, NOT asset value
