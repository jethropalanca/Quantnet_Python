...
class LoanPool(object):
    ...
    def __init__(self):
        ...
        # NO NEED: Implemented this elsewhere...
        # self._t = 0
        # self._totalPrinAsset = {self._t: 0}  # what is left after paying from principalrecfromLP. # ALWAYS THE SAME
        # self._balanceAsset = {self._t: 0}  # what is left after paying from principalrecfromLP.
        # self._pDueAsset = {self._t: 0}  # what is left after paying from principalrecfromLP.
        # self._iDueAsset = {self._t:0}
        # self._monthpayAsset = {self._t: 0}  # what is left after paying from principalrecfromLP.
        # self._activeLoanCount = {self._t: 0}  # what is left after paying from principalrecfromLP.
    ...
    def checkDefaults(self, t):
        '''
        Returns:
        Sum of Recovery Values for the loans in Loan Pool.

        Process:
        Generates a list of uniform random integers, one for each loan in the loan pool.
        The integer range is selected such that the odds of a given number occurring is the same
        odds as the default probability for the time period (always start the range from zero).
        This method then calls checkDefault on each Loan, and passes in a random number.
        '''

        # Generated a list of uniform random integers (one for each Loan).
        if not t:
            recoveryValues = 0  # Else t = 0, and checkDefaults gets an error

        else:
            recoveryValues = 0
            for loan in self._loans:
                randomNumber = random.choice(list(range(int(1 / loan.convtProb(t)))))
                recoveryValues = recoveryValues + loan.checkDefault(randomNumber, t)

        return recoveryValues

    # totalAssetValue
    # def totalassetValue(self, t):
    #     This function returns the sum of the asset values of the loans entered.
    #     # Version 3: Post - AP Comments
    #     lList = self.loanList()
    #     logging.debug('\nCalculating total balance of loans in loan pool...')  # Changed in 4_2_3
    #     return sum(l._asset.value(t) for l in lList)
    #     (self._asset.value(t) * 0.4) if self._hasDefaulted is True else self._asset.value(t)

    def resetdefaultparams(self):
        '''
        This ensures principal payments are not changed during every simulation. Need to reset Loan classes so that none
        of them are in default when a new simulation has been initialized.
        '''

        for loan in self._loans:
            loan._defProb = 0  # default probability
            loan._hasDefaulted = False

        return logging.info('Loans in Loan List have each been reset to default probability zero and to no default state.')